...
def parse_chunks(path: str):
    ...
    while pos < len(data):
        ...
        body_len = complen if complen > 0 else uncomplen
        body_end = body_start + body_len
        if body_end > len(data):
            raise ValueError(
                f"Truncated {name!r} chunk at offset {pos}: "
                f"need {body_len} bytes, only {len(data) - body_start} remain"
            )

        if complen == 0:
            raw = data[body_start:body_end]
        else:
            comp = data[body_start:body_end]

            # Modern Roblox binary places may use either raw LZ4 blocks or
            # Zstandard frames. The compression type is identified by the
            # first four bytes of the compressed payload.
            if comp[:4] == b"\x28\xb5\x2f\xfd":
                if zstd is None:
                    raise RuntimeError(
                        "This .rbxl contains Zstandard-compressed chunks. "
                        "Install the dependency with: pip install zstandard"
                    )
                try:
                    raw = zstd.ZstdDecompressor().decompress(
                        comp, max_output_size=uncomplen
                    )
                except Exception as exc:
                    raise RuntimeError(
                        f"Zstandard decompression failed for chunk {name!r} "
                        f"(compressed={complen}, expected={uncomplen}): {exc}"
                    ) from exc
            else:
                try:
                    raw = lz4.block.decompress(
                        comp, uncompressed_size=uncomplen
                    )
                except Exception as exc:
                    raise RuntimeError(
                        f"LZ4 decompression failed for chunk {name!r} "
                        f"(compressed={complen}, expected={uncomplen}). "
                        f"First 8 payload bytes: {comp[:8].hex()}: {exc}"
                    ) from exc

            if len(raw) != uncomplen:
                raise RuntimeError(
                    f"Decompressed {name!r} chunk to {len(raw)} bytes, "
                    f"but header says {uncomplen}"
                )
